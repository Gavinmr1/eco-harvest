...
export default function Profile() {
  ...
  if (isLoading) {
    return <PageLoaderGate label="Loading Profile..." />;
  }
  ...
  return (
    <main className="text-foreground dark:text-secondary-foreground gap-appSpacing flex flex-col py-[calc(var(--appSpacing)*2)]">
      ...
      <PageHeader
        title="Your Profile"
        subtitle="Manage your contact details and keep your weekly subscription exactly how you like it."
      />
      ...
      <section className="px-appSpacing relative z-10 mx-auto grid w-full max-w-4xl gap-4 lg:grid-cols-2">
        ...
        >
          <div className="flex grow flex-col gap-2">
            <Typography as="h2">Contact & Delivery</Typography>
        ...
        {subscription.subscriptionPlan ? (
          <div className="border-background-border/20 gap-appInnerSpacing flex flex-col rounded-2xl border bg-white/10 p-6 shadow-md backdrop-blur-lg">
            <Typography as="h2">Your Subscription</Typography>
            <div>
              <Typography as="h3">Plan</Typography>
              <Typography as="p" variant="muted">
                {subscription.subscriptionPlan.replace("-week", "-week plan")}
              </Typography>
            </div>
            <div>
              <Typography as="h3">Status</Typography>
              <div className="flex flex-wrap items-center justify-between gap-4">
                <Typography as="p" variant="muted">
                  {statusLabel}
                </Typography>
                {subscription.statusUpdatedAt ? (
                  <Typography as="p" className="bg-background/60 w-fit rounded-full px-3 py-1">
            ...
            <div>
              <Typography as="h3">Box Size</Typography>
              <Typography as="p" variant="muted">
                {subscription.boxSize || "Not selected"}
              </Typography>
            </div>
            <div>
              <Typography as="h3">Preferences</Typography>
